...
class Question extends Component {
    ...
    delayImage = () => {
        ...
        blankImg.src = 'http://knowapp.b0.upaiyun.com/ss/pcSide/fill_image_samll.png';
        setTimeout(() => {
            document.getElementsByName('questionFillImage').forEach((e) => {
                let img = new Image();
                img.onload = function () {
                    let ctx = e.getContext("2d");
                    ctx.drawImage(img, 0, 0);
                    let pos = JSON.parse(e.dataset.position);
                    let answer = e.dataset.answer == 'undefined' ? [] : e.dataset.answer.split(',');
                    if (answer.length === 0) {
                        for (let key in pos) {
                            ctx.drawImage(blankImg, pos[key].x_pos + 25, pos[key].y_pos + 7);
                        }
                    }
                    answer.forEach((val, index) => {
                        ctx.font = "20px PingFangSC-Regular";
                        ctx.fillStyle = "#80d74a";
                        ctx.fillText(val, pos[`imgBlankAnswer${index}`].x_pos + 40, pos[`imgBlankAnswer${index}`].y_pos + 35);
                    })
                };
        ...
        setTimeout(() => {
            document.getElementsByName('question').forEach((img) => {
    ...
    render() {
        ...
        let data = {};
        data = reverseParseTextView(originData);
        let difficulty = ['未知', '容易', '一般', '较难', '难'];
        return (
